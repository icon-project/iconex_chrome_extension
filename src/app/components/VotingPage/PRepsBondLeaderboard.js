...
@withLanguageProps
class PRepsBondLeaderboard extends Component {
  state = {
  ...
  render() {
    ...
    const { checked, keyword } = this.state;
    const wrapClassName = isBondMode ? "choice" : "";
    const pRepsFiltered = pReps
      .filter((pRep) => checked[pRep.grade])
      .filter((pRep) =>
        pRep.name.toLowerCase().includes(keyword.toLowerCase())
      );
...
const SearchCheckBox = ({ id, label, cnt, checked, filterData }) => (
  <span>
    ...
    />
    <label htmlFor={`cbox-0${id}`} className="label _img">
      {label}
      <em>({cnt})</em>
    </label>
  </span>
...
export default PRepsBondLeaderboard;
