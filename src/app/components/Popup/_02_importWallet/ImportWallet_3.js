    ...
    this.setState(
      ...
      () => {
        if (state === "submit" && !(privateKeyError || pwError)) {
          ...
          this.props.setCoinType(
            this.state.coinTypeIndex === 0 ? "icx" : "eth"
          );
      ...
      <div>
        <div className="header">
          <span className="close">
            <em onClick={this.closePopup} className="_img"></em>
          </span>
          ...
          <div className="scroll">
            <div className="tabbox-holder">
              <div className="name-group abs">
                ...
                <MoneyGroup
                  types={CoinTypes}
                  index={this.state.coinTypeIndex}
                  selectCoinType={this.selectCoinType}
                />
              </div>
              <div className="pw-group" style={{ zIndex: 1 }}>
                <p className="title">{I18n.importWallet.inputLabel3_2}</p>
                <input
                  onKeyPress={this.handleKeyPress}
                  onChange={this.changeInput}
                  type="text"
                  className={`txt-type-normal ${privateKeyError && "error"}`}
                  placeholder={I18n.importWallet.inputPlaceHolder3_2}
                  name="privateKey"
                  value={privateKey}
                  spellCheck="false"
                />
        ...
        <div className="btn-holder">
          <button
            ...
            type="submit"
            className="btn-type-next size-full"
          >
            <span>{I18n.button.import}</span>
          </button>
        </div>
      </div>
    );
