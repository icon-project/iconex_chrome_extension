...
class PRepsBar extends React.Component {
  ...
  viewDetail = () => {
    const { isVoteMode, data: { address } } = this.props
    if (!isVoteMode) {
  ...
  render() {
    ...
    const getLabel = () => {
      let result = !isUnregistered ? `${Object.values(P_REP_TYPE)[grade]}` : 'Unregistered'
    ...
    return (
      <tr
            ...
            isSelected && (
              <div className="controller-group">
                <PRepsBarInputRange
                  ...
                  step={step}
                />
                <p>{I18n.votePage.max}<em>{`${maxAvailablePct}%`}</em></p>
        ...
        {!isLeaderboard && !isVoteMode && (
          <td>{`${convertStakeValueToText(myDelegation)} (${myDelegationPct}%)`}</td>
        )}
      </tr>
    )
  }
